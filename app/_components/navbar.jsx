...
export default function Navbar() {
  ...
  return (
    <nav
      id="nav-div"
      className={`w-full z-90 opacity-70 py-3 tracking-wide text-2xl not-arrow-up fixed dm-serif-text-bold
      ${
        isVisible
          ? "bg-white border-b border-b-mako-600 opacity-100"
          : "bg-transparent"
      }`}
    >
      <div className="mx-auto relative md:px-10 w-full items-center justify-between px-4 grid grid-cols-[minmax(60px,1fr)_minmax(200px,4fr)_minmax(15px,0.5fr)] lg:grid-cols-[minmax(400px,3fr)_minmax(40px,1fr)_minmax(400px,3fr)]">
        ...
        <div
          className={`flex content-center lg:justify-center justify-start min-w-[50px] ${isVisible? 'fill-mako-900' : 'fill-mako-800'}`}
          id="phoenix-logo"
        >
        ...
        <div className="hidden lg:inline  items-center min-w-[402px] px-2">
          <div className="space-x-2 justify-start items-center text-mako-500">
            <a
              href="/pour-qui"
              className={`rounded-md font-light text-md  ${"  hover:text-mako-500 hover:bg-mako-50 px-2 py-1"}`}
            >
            ...
            <a
              ...
              className="bg-mantis-500 font-bold text-mantis-50 px-5 py-3 rounded-3xl inline ml-8"
            >
              Doctolib
            </a>
          </div>
